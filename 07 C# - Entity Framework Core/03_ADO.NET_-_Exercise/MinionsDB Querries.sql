...
-----
---Return Villian Name By Id
SELECT Name 
	FROM Villains
	WHERE Id = 1

--Return All Minion Info About Minion
SELECT m.Name,m.Age
	FROM Villains AS v
...
SELECT Id FROM EvilnessFactors WHERE Name = 'Evil'
...
DELETE FROM MinionsVillains WHERE MinionId = 1 AND VillainId = 1
...
SELECT Name,Age FROM Minions WHERE Id = @minionId
  ...
  SELECT *
	...
	SELECT COUNT(*) FROM Countries AS c LEFT JOIN Towns as t ON t.CountryCode = c.Id WHERE c.Name = @countryName AND t.Name IS NULL